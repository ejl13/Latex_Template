...
Additionally for future experiments, it is worth noting strengths and limitations of the two methods employed. As long as the magnetic field is reasonably strong, and one collects enough data points, the direct method offers a very accurate answer because when fitting the $\cos^2$ function, the errors in each data point constrain each point, improving the overall error by $\frac{1}{sqrt{N}}. However, since there is a clear limit on how precisely one can change the angle on the polarizer - in our case we were accurate to within a degree, but only in 5 degree increments - dealing with very small magnetic fields or much smaller Verdet Constants would prove problematic. In that case, it is worth locking the rotating polarizer at a relative angle of 45 degrees and varing the current supplying the magnetic field, since the equipment used in this experiment can produce a current of as small as 0.001A and can measure photodiode voltages down to 0.001mV, and such precision allows for the acquisition of very small changes. In fact, if one were to modulate the magnetic field as $B = B_0+B_1\sin{\omega t}$ for $B_1 \ll B_0$, then the change in photodiode voltage will directly be the slope $\frac{dV}{dB}$.

Speaking of variations in the magnetic field, it is worth noting that we only took a simple average over the range of the glass rod to acquire an approximate value of field strength. This may have sufficed for our purposs, since it is observed that field strength does not vary much within the glass. However, if we aim to measure the Verdet constant for a liquid sample that extends outside the solenoid, the simple average would not give us a reliable estimate on field strength. In that case, we need to use the weighted average $\frac{1}{N} \sum_{x_1=-L/2}^{x_N = L/2}B(x_i)$ instead. Even better, we can fit data to a smooth function for B(x) (not feasible in our case given the insufficient number of data points), and use the formula $\frac{1}{L} \int_{-L/2}^{L/2}B(x)dx$ for a better estimate of field strength.

Given that we are also unsure of the exact wavelengeth dependence, the experiment could be done with the above constant relative angle of 45 degrees and a fixed magnetic field, but to oscillate the frequency of the laser instead and measure the effect it has on $\frac{dV}{dB}$.

In the end, the Faraday Effect is worth studying and understanding well, as it has numerous applications. One example of such is the optical isolator, or optical diode. It works much like our experiment - light light passes through a polarizer, has its polarization angle changed passing through a material under a magnetic field to align with a second polarizer (at a 45 degree angle relative to the first polarizer) at the other end so that the light can pass through. The effect is that any reflected light will now be at 90 degrees relative to the first polarizer and thus will be unable to pass through. This is extremely useful in laser cavities. Alternatively, on can use this to change the polarization of light coming from different directions without interfering with eachother, as in fiber optic cables.
